...
const ToastViewport = forwardRef<
    ...
    ComponentPropsWithoutRef<typeof ToastPrimitives.Viewport> & { variant?: 'bottom-left' | 'top-center' }
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Viewport
        ref={ref}
        className={cn(
            'fixed top-0 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:flex-col md:max-w-[420px]',
            props.variant && props.variant === 'bottom-left'
                ? 'sm:bottom-0 sm:left-0 sm:top-auto'
                : 'sm:left-1/2 sm:top-0 sm:transform sm:-translate-x-1/2',
            className
        )}
        {...props}
    />
...
const toastVariants = cva(
    'data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none',
    {
        variants: {
            variant: {
            ...
            direction: {
                toLeft: 'data-[state=closed]:slide-out-to-left-full',
                toTop: 'data-[state=closed]:slide-out-to-top-full',
            },
        },
        defaultVariants: {
            variant: 'default',
...
>(({ className, variant, direction, ...props }, ref) => {
    return (
        <ToastPrimitives.Root ref={ref} className={cn(toastVariants({ variant, direction }), className)} {...props} />
    )
})
