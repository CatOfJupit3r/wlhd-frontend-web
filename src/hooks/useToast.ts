...
export const reducer = (state: State, action: Action): State => {
    switch (action.type) {
        case 'ADD_TOAST': {
            ...
            return {
                ...state,
                toasts: [action.toast, ...state.toasts].filter((toast) => {
                    const position = toast.position || 'top-center';
                    if (encounters[position] >= TOAST_LIMITS[position]) {
                        return false;
                    }
                    encounters[position]++;
                    return true;
...
function toastError({ title, description }: { title: string; description: string }) {
    return toast({ title, description, variant: 'destructive', position: 'top-center' });
}
